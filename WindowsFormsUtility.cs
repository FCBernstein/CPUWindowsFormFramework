...
namespace CPUWindowsFormFramework
{
    public class WindowsFormsUtility
    {
        ...
        {
            string propertyname = "";
            string contolname = ctrl.Name.ToLower();
            string controltype = contolname.Substring(0, 3);
            string columnname = contolname.Substring(3);
        ...
        public static int GetIdFromGrid(DataGridView grid, int rowindex, string colomnname)
        {
            int id = 0;
            if (rowindex < grid.Rows.Count && grid.Columns.Contains(colomnname) && grid.Rows[rowindex].Cells[colomnname].Value != DBNull.Value)
            {
                if (grid.Rows[rowindex].Cells[colomnname].Value is int)
                {
                    id = (int)grid.Rows[rowindex].Cells[colomnname].Value;
                }
            }
            return id;
        }
